...
async fn mesh_infer(
    ...
    // Router selon le type de plan
    let result = match &plan {
        ainonymous_types::ExecutionPlan::Solo { node, .. } => {
            let node_id = node.clone();
            crate::conductor::run_solo_inference(
                &s.conductor.llama,
                &body.model_id,
                body.messages,
                body.max_tokens,
...
///
/// Palier E/F — Rotation de clé ed25519 du nœud :
/// 1. Génère une nouvelle clé, l'écrit dans `identity_path`
/// 2. Re-annonce la nouvelle clé publique dans le DHT
/// 3. Émet de nouveaux warrants (ModelClaim + NodeCapabilities) de façon sûre
///    (non-fatale : la rotation réussit même si le zome 'warrants' échoue)
///
/// La connexion mTLS QUIC actuelle reste active avec l'ancienne clé jusqu'au
/// prochain redémarrage du daemon.
///
/// Réponse :
/// ```json
/// {
...
async fn rotate_identity(State(s): State<DaemonState>) -> impl IntoResponse {
    ...
    if let Err(e) = s.holochain.try_emit_node_capabilities(&new_identity).await {
