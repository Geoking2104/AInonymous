...
- Patch natif llama.cpp (pipeline-split par couche, sans dépendance Python), Gemma3 Dense : ✅ preuve de concept réelle et vérifiée (voir `patches/llama-cpp-pipeline-split/`) —
  ...
  - **`pipeline_layer_end` (early-exit, nécessaire pour N>2 nœuds) : bug diagnostiqué, corrigé, et branché dans `pipeline_server.cpp`** — cause racine trouvée par instrumentation (`ggml_backend_buffer_get_type` appelé sur un tenseur `inp_out_ids` jamais alloué car construit avant que la portée early-exit ne soit connue dans `gemma3.cpp`). Fix d'une ligne (construction conditionnelle), pas de bug du scheduler ggml. Vérifié bit-exact (34658/34658) sans régression sur les tests existants. **Le serveur HTTP appelle maintenant réellement `llama_set_pipeline_layer_end()`** (avant : acceptait `--layer-end` mais ne l'exploitait que pour l'extraction, recalculait le modèle complet) — économie de calcul confirmée par les logs de reserve : graphe ggml de node0 passe de 87 à 42 nœuds, tokens toujours bit-exact. Reste à vérifier : un vrai nœud du milieu (`layer_start>0` + `layer_end<n_layer` simultanément) sur un modèle à 3+ couches (le modèle de test n'a que 2 couches)
