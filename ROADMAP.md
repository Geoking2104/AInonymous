...
- Patch natif llama.cpp (pipeline-split par couche, sans dépendance Python), Gemma3 Dense : ✅ preuve de concept réelle et vérifiée (voir `patches/llama-cpp-pipeline-split/`) —
  ...
  - **Vrai nœud du milieu (`layer_start>0` ET `layer_end<n_layer` simultanément, nécessaire pour N>2 nœuds) : testé et vérifié le 27/07/2026** — modèle de test à 3 couches généré sans torch/transformers (script `build_3layer_test_gguf.py`, copie directe des tenseurs GGUF), chaîne réelle à 3 process HTTP (node0 first + node1 MIDDLE + node2 last) bit-exact avec un baseline single-contexte, économie de calcul confirmée sur le nœud du milieu aussi (127→42 nœuds ggml)
  - restent : MoE (Gemma4/gemma3n), GPU, build automatisé du binaire natif (pas encore vendoré/scripté), run réel du testnet natif avec cargo, concurrence serveur, chaînes à 4+ nœuds (jamais essayées, devrait marcher par construction mais non vérifié)
